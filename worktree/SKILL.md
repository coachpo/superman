---
name: worktree
description: Create, list, and remove git worktrees with automatic environment setup. Use when Codex needs to manage sibling model-share branch worktrees, copy .env, initialize submodules, and run pnpm install.
---
...
Run the CLI at `worktree/cli.mjs`. Do **not** use the old `.claude/skills/worktree/cli.mjs` path.

```bash
# Create a worktree for a new branch
node worktree/cli.mjs create feature/my-feature

# Create a worktree for an existing branch
node worktree/cli.mjs create existing-branch

# List all worktrees
node worktree/cli.mjs list

# Remove a worktree
node worktree/cli.mjs remove feature/my-feature
```
...
node worktree/cli.mjs create feature/user-auth
# Creates: ../model-share-feature-user-auth

# Create worktree for a bugfix
node worktree/cli.mjs create fix/login-issue
# Creates: ../model-share-fix-login-issue

# Remove when done
node worktree/cli.mjs remove fix/login-issue
```
...
- The CLI resolves the git repository from your current working directory, so run it from inside the repo or worktree you want to manage

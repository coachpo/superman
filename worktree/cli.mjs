...
function resolveGitPath(args, description) {
  try {
    ...
    console.error('Run this command from inside the repository or worktree you want to manage.');
    process.exit(1);
  }
}

const CURRENT_WORKTREE = resolveGitPath('--show-toplevel', 'Not inside a git worktree.');
const GIT_COMMON_DIR = resolve(process.cwd(), resolveGitPath('--git-common-dir', 'Unable to resolve the shared git directory.'));
const MAIN_WORKTREE = dirname(GIT_COMMON_DIR);
